...
一个 PR 提交到默认分支或其他目标分支后，会同时形成两类检查：
...
默认分支是统一 Router 入口。无论 PR 的目标是默认分支还是其他分支，授权和 Worker 选择都由默认分支中的可信 Router 完成；Router 不 checkout PR 代码，也不读取 `GITEE_TOKEN`。

| PR 场景 | Router 的处理 | 实际执行 Local CI 的 Worker |
| --- | --- | --- |
| PR 提交到默认分支 | 默认分支只负责路由和授权，不直接执行候选代码 | 按配置选择专用/fallback Worker |
...
这里的关键区别不是“默认分支是否测试”，而是“谁提供可信 Worker”。PR 的候选代码始终作为被测对象，不会替换 Router、scanner、dispatcher 或本地 supervisor。
...
- 当前配置模板默认允许 write token fallback；自动处理外部 fork PR 时，应显式将上述开关设为 `0`，或为容器提供最小权限的 relay token；
...
| 机制 | 作用 |
| --- | --- |
| 外部 fork Environment 审批 | 配置 Required reviewers 后，阻止未授权外部 PR 自动消耗本地资源 |
...
这些机制用于抑制重复执行、旧任务回写和未授权外部任务。

### 4.4 安全配置检查表

- `local-ci-fork-approval` 已配置 Required reviewers；
- 自动处理外部 fork PR 时，显式设置 `LOCAL_CI_ALLOW_WRITE_TOKEN_IN_CONTAINER=0`，或为容器提供最小权限 relay token；
