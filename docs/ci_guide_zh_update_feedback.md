...
Ruff、纯 Python 测试、脚本预检和 API 对比在 GitHub-hosted runner 上快速执行。前端重装、后端 rebuild、smoke/JIT、FlagGems 和性能测量依赖本地工具链，由本地服务器完成。
...
- 默认分支负责路由和授权；Worker 负责安全检查、投递、接收结果和页面刷新。
...
CI Gateway 把路由和执行拆开。默认分支的 Router 接收 `pull_request_target`，处理外部 fork 审批、选择 Worker、取消旧任务和手动跨分支 push。普通目标分支的 Worker 运行 Security Gate、dispatcher、receiver、Pages 和分支自己的 GitHub CI。

Router 只运行默认分支中的可信代码：不 checkout 或执行 PR 内容，也不读取 `GITEE_TOKEN`。Gitee 凭据只进入 Worker 执行链；PR dispatch 会在 Security Gate 通过后才使用它。

目标分支没有 manifest 时，Router 可将 PR 交给 fallback Worker；当前默认使用 `CI_dev`。Gitee 只保存 task ref 和结果，本地服务器只负责发现任务、运行测试和发布结果。
...
编译或后端失败时，Local CI 只收集本次失败命令生成的 `.ttir`、`.linalg`、`.pplir`，让定位时看到的 IR 与本次失败直接对应，避免混入旧任务或全局缓存中的文件。
