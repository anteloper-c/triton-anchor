...
- `Mirror RACE-org Pull Requests` 每天读取上游全部 open PR，默认只镜像目标分支为 `main`、`triton_v3.0`、`anchorbase_dev` 或 `CI_dev` 的 PR，把实时 base 和精确 head SHA 推送到 `review/race-pr-<编号>/base` 与 `review/race-pr-<编号>/head`，再在 fork 内创建审核 PR。
...
镜像分支白名单通过 workflow 输入 `allowed_base_refs` 或仓库变量 `MIRROR_UPSTREAM_ALLOWED_BASE_REFS` 配置，默认值为 `main,triton_v3.0,anchorbase_dev,CI_dev`。填写 `*` 或 `all` 表示镜像所有上游目标分支；在后续按 Triton 版本动态选择 LLVM/profile 的逻辑就绪前，不建议打开全量分支。
...
python scripts/local_ci/upstream_pr_mirror/mirror_upstream_prs.py \
  ...
  --allowed-base-refs main,triton_v3.0,anchorbase_dev,CI_dev \
  --pr-number 66 \
  --dry-run
```
