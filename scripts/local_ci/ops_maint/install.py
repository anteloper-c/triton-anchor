...
def render_units(
    ...
    health = f"{python} {quoted(str(root / 'ops_maint/health.py'))} --config {quoted(str(config_path))} --publish"
    retention = f"{python} {quoted(str(root / 'ops_maint/retention.py'))} --config {quoted(str(config_path))} --apply"
    units = {
    ...
    watchdog = f"{python} {quoted(str(root / 'ops_maint/watchdog.py'))} --config {quoted(str(config_path))} --publish"
...
def install_units(units: dict[str, str], destination: Path, backup: Path) -> dict:
    ...
    manifest = {
        "schema": "triton-anchor-local-ci-service-install/v2",
        "scope": "user",
        "uid": os.getuid(),
...
def rollback_units(backup: Path, *, apply: bool = False) -> dict:
    ...
    if (
        manifest.get("schema") != "triton-anchor-local-ci-service-install/v2"
        or manifest.get("scope") != "user"
    ):
