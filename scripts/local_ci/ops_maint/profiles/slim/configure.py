...
from ops_maint.artifacts import tree_digest
from ops_maint.dependency_mounts import dependency_mounts
...
def main():
    ...
    profile["env"]["BACKEND_TEST_COMMAND"] = (
        ...
        '"$PYTHON_BIN" -I /opt/local-ci/control/scripts/local_ci/ops_maint/profiles/slim/validate_flaggems.py'
    )
    profile["prepare_commands"] = [
        [
            "ln",
            "-s",
            "deps/llvm-" + profile["llvm_hash"],
            "/opt/local-ci/runtime/llvm-release",
        ],
        ["ln", "-s", "deps/ppl", "/opt/local-ci/runtime/ppl-release"],
        [
            "/opt/venv/bin/python",
            "-I",
            "/opt/local-ci/control/scripts/local_ci/ops_maint/profiles/slim/enable_flaggems.py",
            "--source",
            target,
        ],
