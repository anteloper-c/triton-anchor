...
class DeploymentTests(unittest.TestCase):
    def setUp(self):
        ...
        self.config = {
            "control_root": str(DEPLOY.parents[2]),
    ...
    def test_rendering_does_not_start_services_and_uses_private_env(self):
        units = installer.render_units(
            self.config, self.root / "config.json", self.root / "credentials.env"
        )
        ...
        self.assertNotIn("triton-anchor-local-ci-environment-fixture.timer", units)
        self.assertIn("triton-anchor-local-ci-watchdog.timer", units)
        self.assertNotIn(
            "triton-anchor-local-ci.service",
            units["triton-anchor-local-ci-health.service"],
        )
    ...
    def test_rollback_preserves_third_party_edits(self):
        ...
        installer.install_units(
            {"triton-anchor-local-ci.service": "new"}, units, backup
        )
    ...
    def test_ci_user_may_have_manual_sudo_but_not_rootful_runtime_groups(self):
        ...
        for name, expected in (
            ...
            with (
                self.subTest(group=name),
                patch.object(preflight.pwd, "getpwuid", return_value=account),
                patch.object(
                    preflight.grp,
                    "getgrall",
                    return_value=[
                        SimpleNamespace(gr_name=name, gr_gid=999, gr_mem=["ci_trial"])
                    ],
                ),
                ...
                patch.object(
                    preflight,
                    "runtime_status",
                    side_effect=ValueError("No real Docker during test"),
                ),
            ):
                report = preflight.check_configuration(
                    config, runtime=True, require_notifications=False
                )
            status = next(
                row["status"]
                for row in report["checks"]
                if row["check"] == "ci_account_groups"
            )
            self.assertEqual(expected, status)

    def test_sample_fails_missing_production_values(self):
        config = json.loads((DEPLOY / "config.example.json").read_text())
        result = preflight.check_configuration(
            config, runtime=False, require_notifications=False
        )
        self.assertFalse(result["ready"])
        failed = {
            entry["check"] for entry in result["checks"] if entry["status"] == "fail"
        }
        self.assertIn("gitee_repo_url", failed)
        self.assertIn("profile:triton_v3.0:image", failed)
        self.assertIn("profile:triton_v3.0:image_validation", failed)
    ...
    def test_collection_detects_stopped_poller_independently(self):
        ...
        (state / "health/worker.json").write_text(
            json.dumps(
                {"worker_id": "fixture", "heartbeat_at": 1000, "pid": os.getpid()}
            )
        )
    ...
    def test_one_way_configuration_rejects_receipt_timeout_and_checks_retention(self):
        ...
        def check(name):
            ...
            return next(row for row in result["checks"] if row["check"] == name)[
                "status"
            ]
    ...
    def test_workspace_retention_defaults_and_numeric_boundaries(self):
        config = json.loads((DEPLOY / "config.example.json").read_text())
        defaults = {
            "task_workspace_max_bytes": 100 * 1024**3,
    ...
    def test_branch_profile_mapping_is_checked_before_deployment(self):
        ...
        for value in (
            ...
            with self.subTest(value=value):
                ...
                self.assertEqual(
                    "fail", self.configured_checks(config)["branch_profiles"]
                )
    ...
    def test_runtime_preflight_rejects_root_worker(self):
        ...
        row = next(
            row for row in result["checks"] if row["check"] == "ordinary_ci_user"
        )
        ...
        for branch, profile in config["profiles"].items():
            ...
            for value in (True, False, None, 1, "true"):
                with self.subTest(branch=branch, value=value):
                    ...
                    expected = (
                        "pass" if type(value) is bool and value == required else "fail"
                    )
                    self.assertEqual(expected, self.configured_checks(config)[name])
            del profile["backend_enabled"]
            self.assertEqual(
                "fail" if required else "pass", self.configured_checks(config)[name]
            )
    ...
    def test_preflight_checks_actual_finish_budget_without_host_sessions(self):
        ...
        for value in (719, 0, -1, True, 1.5, "3600", None, 86301):
            with self.subTest(deadline=value):
                config["finish_timeout_seconds"] = value
                self.assertEqual(
                    "fail", self.configured_checks(config)["finish_timeout_seconds"]
                )
        config["finish_timeout_seconds"] = 720
        self.assertEqual(
            "pass", self.configured_checks(config)["finish_timeout_seconds"]
        )
    ...
    def test_health_does_not_treat_recipe_roots_as_host_storage(self):
        config = {
            **self.config,
            "profiles": {"main": {"workspace_root": "/image/recipe/root"}},
        }
        ...
        self.assertNotIn("/image/recipe/root", json.dumps(result))
