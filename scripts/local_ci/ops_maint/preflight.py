...
from ops_maint.manager import DIGEST_RE, NAME_RE, SHA_RE, safe_source
from agent_ci.protocol import ContractError
from agent_ci.codex import finish_timeout_seconds
from ops_maint.runtime_probe import (
    runtime_status,
    verify_probe,
    probe_runtime,
    validate_runtime_config,
)
from ops_maint.dependency_mounts import dependency_mounts, validate_mounted_llvm
from ops_maint.runtime import validate_branch_profiles
...
def check_configuration(
    ...
    check(
        "schema",
        config.get("schema") == "triton-anchor-local-ci-config/v2",
        "Use the explicit v2 Rootless Docker task-container configuration",
    )
    check(
        "removed_host_codex_user",
        "codex_user" not in config and "container_execution_user" not in config,
        "Remove host codex_user/container_execution_user; configure one non-root task identity",
    )
    check(
        "codex_bin",
    ...
    check(
        "one_way_delivery",
        "receipt_timeout_seconds" not in config,
    ...
    try:
        ...
        check(
            ...
            True,
            "Bounded task sealing deadline configured: " + str(deadline) + " seconds",
        )
        ...
        if home_valid:
            try:
                ...
                check(
                    "company_provider",
                    True,
                    "Existing company model/provider files validated; no API request made",
                )
            except (Exception, SystemExit):
                check(
                    "company_provider",
                    False,
                    "Actual company model/provider configuration failed validation",
                )
        try:
            ...
            check(
                "runtime_probe",
                True,
                "Verified effective resource limits and trusted image capabilities: "
                + evidence["config_digest"],
            )
        except (OSError, ValueError, RuntimeError, subprocess.TimeoutExpired) as exc:
            check("rootless_runtime_or_probe", False, str(exc))
    return {
        "schema": "triton-anchor-local-ci-preflight/v2",
