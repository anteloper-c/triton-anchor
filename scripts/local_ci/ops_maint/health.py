...
from ops_maint.manager import EnvironmentManager, atomic_json, safe_source
from ops_maint.runtime_probe import runtime_status
...
def collect(config: dict, *, now: float | None = None, manager=None) -> dict:
    ...
    try:
        ...
        workspaces = {
            "status": "unreported",
            "reason": "No task workspace cleanup snapshot has been recorded",
        }
    except (OSError, ValueError) as exc:
        workspaces = {
            "status": "error",
            "error": type(exc).__name__,
            "reason": "Task workspace cleanup snapshot is unreadable",
        }
    services = []
