...
DEFAULT_IDENTITIES = {"candidate": 11001, "base": 11002, "diagnostic": 11003, "codex": 11004,
                      "read_gid": 11000, "codex_gid": 11004}
...
def validate_runtime_config(config):
    ...
    if not isinstance(runtime, dict):
        raise ValueError("runtime must be an explicit Rootless Docker configuration object")
    ...
    if not isinstance(endpoint, str) or not re.fullmatch(r"unix:///run/user/[1-9][0-9]*/[A-Za-z0-9_.-]+\.sock", endpoint):
        raise ValueError("Configure the actual private Rootless socket under /run/user/<CI UID>; system Docker fallback is forbidden")
    if not isinstance(context, str) or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]*", context):
    ...
    if type(cpus) not in (int, float) or not 0 < cpus < 100000 or not math.isfinite(cpus):
        raise ValueError("resources.cpus must be a finite positive number")
    if any(type(resources.get(name)) is not int or resources[name] <= 0 for name in ("memory_bytes", "pids_limit")):
        raise ValueError("resources.memory_bytes and resources.pids_limit must be positive integers")
    ...
    if any(type(value) is not int or not 0 < value < 65536 for value in identities.values()):
        raise ValueError("Container identities must be explicit non-root integer UIDs/GIDs")
    if len({identities[name] for name in ("candidate", "base", "diagnostic", "codex")}) != 4 or identities["read_gid"] == identities["codex_gid"]:
        raise ValueError("Candidate/base/diagnostic/Codex UIDs and private Codex/read groups must be distinct")
    return runtime, resources, identities


def docker(config, *args, timeout=30):
    runtime, _, _ = validate_runtime_config(config)
    environment = {key: value for key, value in os.environ.items() if key not in {"DOCKER_HOST", "DOCKER_CONTEXT", "DOCKER_TLS_VERIFY", "DOCKER_CERT_PATH"}}
    result = subprocess.run([config.get("docker_bin", "docker"), "--host", runtime["endpoint"], *args],
...
def runtime_status(config):
    runtime, _, _ = validate_runtime_config(config)
    if os.geteuid() == 0:
        raise ValueError("Rootless preflight/probe must run as the ordinary CI user without sudo")
    ...
    if not isinstance(context, list) or len(context) != 1 or context[0].get("Endpoints", {}).get("docker", {}).get("Host") != runtime["endpoint"]:
        raise ValueError("Configured Docker context points at a different endpoint")
    info = json.loads(docker(config, "info", "--format", "{{json .}}"))
    if not any("rootless" in str(value).split(",") or "name=rootless" in str(value).split(",") for value in info.get("SecurityOptions", [])):
    ...
    if str(info.get("CgroupVersion")) != "2" or info.get("CgroupDriver") != "systemd":
        raise ValueError("Effective limits require cgroup v2 and the systemd cgroup driver")
    if any(info.get(name) is True for name in ("NoCpuCfsQuota", "NoCpuCfsPeriod", "NoMemoryLimit", "NoPidsLimit")):
    ...
    return {"daemon_id": info["ID"], "endpoint": runtime["endpoint"], "uid": os.getuid(),
            "cgroup_version": str(info["CgroupVersion"]), "cgroup_driver": info["CgroupDriver"],
            "kernel_version": info.get("KernelVersion", ""), "server_version": info.get("ServerVersion", "")}
...
def config_digest(config):
    ...
    return hashlib.sha256(json.dumps(public, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode()).hexdigest()
...
def active_images(config):
    ...
    for branch in config.get("profiles", {}):
        ...
        if (not release or not re.fullmatch(r"sha256:[a-f0-9]{64}", release.get("image_id", "")) or release.get("validated") is not True
                or release.get("state") not in {"ready", "active"} or release.get("target_branch") != branch
                or release.get("llvm_hash") != config["profiles"][branch].get("llvm_hash")):
            raise ValueError("Build and validate a trusted image release before probing: " + branch)
...
def validate_limits(actual, resources):
    try:
        ...
        if abs(cpu - resources["cpus"]) > 0.00001 or memory != resources["memory_bytes"] or pids != resources["pids_limit"]:
            raise ValueError("Mismatch")
    except (ValueError, KeyError, TypeError, ZeroDivisionError):
        raise ValueError("Container CPU/memory/PID limits are not effectively enforced") from None


def proof_path(config):
    return Path(config["state_dir"]) / "deploy/runtime-probe.json"
...
def verify_probe(config, info):
    ...
    if (proof.get("schema") != PROBE_SCHEMA or proof.get("runtime") != info
            or proof.get("config_digest") != config_digest(config) or proof.get("images") != expected_images):
        raise ValueError("Runtime/image/resource configuration changed; repeat preflight --probe-runtime")
    if proof.get("status") != "pass" or set(proof.get("limits", {})) != set(expected_images):
...
def probe_runtime(config):
    ...
    proof = {"schema": PROBE_SCHEMA, "status": "running", "runtime": info, "config_digest": config_digest(config),
             "images": images, "resources": resources, "limits": {}, "probed_at": time.time()}
    ...
    try:
        for branch, image in images.items():
            ...
            container = ""
            try:
                container = docker(config, "create", "--name", name, "--label", "triton-anchor.role=deployment-probe", "--label", "triton-anchor.probe=" + nonce,
                ...
                try:
                    inspection = json.loads(docker(config, "inspect", container if re.fullmatch(r"[a-f0-9]{64}", container) else name))
                except RuntimeError:
                    if container:
                        raise
                    inspection = []
                if inspection:
                    ...
                    if (len(inspection) != 1 or not re.fullmatch(r"[a-f0-9]{64}", actual) or (container and actual != container)
                            or inspection[0].get("Config", {}).get("Labels", {}).get("triton-anchor.probe") != nonce
                            or inspection[0].get("Config", {}).get("Labels", {}).get("triton-anchor.role") != "deployment-probe"):
                        raise ValueError("Probe ownership changed; refusing container cleanup")
                    docker(config, "rm", "--force", actual)
        if config_digest(config) != proof["config_digest"] or active_images(config) != images:
            raise ValueError("Trusted control or image release changed during runtime probe")
