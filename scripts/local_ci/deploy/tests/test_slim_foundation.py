...
SLIM = Path(__file__).resolve().parents[1] / "jiwang_ci/slim"
spec = importlib.util.spec_from_file_location("enable_flaggems", SLIM / "enable_flaggems.py")
...
class SlimFoundationTests(unittest.TestCase):
    def test_adds_path_without_import_or_install_when_mount_absent(self):
        with tempfile.TemporaryDirectory() as temporary:
            with patch.object(enable_flaggems.sysconfig, "get_path", return_value=temporary), \
                 patch("sys.argv", ["enable_flaggems", "--source", "/opt/local-ci/runtime/deps/flaggems"]):
                enable_flaggems.main()
            self.assertEqual("/opt/local-ci/runtime/deps/flaggems/src\n",
