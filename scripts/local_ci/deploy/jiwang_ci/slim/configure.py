...
def main():
    ...
    del profile["repositories"]["FlagGems"]
    profile["archives"].pop("codex-cli", None)
    ...
    profile["env"]["BACKEND_TEST_COMMAND"] = (
        ...
        '\"$PYTHON_BIN\" -I /opt/local-ci/control/scripts/local_ci/deploy/jiwang_ci/slim/validate_flaggems.py'
    )
    profile["prepare_commands"] = [
        ["ln", "-s", "deps/llvm-" + profile["llvm_hash"], "/opt/local-ci/runtime/llvm-release"],
        ["ln", "-s", "deps/ppl", "/opt/local-ci/runtime/ppl-release"],
        ["/opt/venv/bin/python", "-I", "/opt/local-ci/control/scripts/local_ci/deploy/jiwang_ci/slim/enable_flaggems.py", "--source", target],
        ["git", "config", "--system", "--add", "safe.directory", target],
    ]
    dependency_mounts(config, profile, verify_content=True)
    backup = path.with_name("local-ci-before-slim-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ") + ".json")
