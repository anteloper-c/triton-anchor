...
限制；只描述实际执行的检查，不罗列全部未选工具。不写任务编号、控制版本、
工具代号或内部状态；这些追溯信息由详细报告保留。未执行但影响合入判断的
关键检查必须明确说明，不能把文档审查或未完成的构建写成编译器验证通过。
只讨论与当前改动相关的架构边界；总结侧重变更目的和结论，单项说明侧重
实际验证的行为与依据，避免重复罗列无关契约。
