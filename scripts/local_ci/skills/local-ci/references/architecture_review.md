...
每次提交架构审查，包括无影响时的依据。可信无语义改动可用实际变更路径、AST 等价分类和“未触及上述契约”作为简洁 no-impact 证据，不遍历无关架构代码。直接规则违规按主流程提供可信 base 规则行号及原文、候选代码行号及原文、直接违反该规则的解释；由 Harness 核验。审查继续使用 `submit_review` 既有状态，不把 `continue` 或 `block` 填入其 `status`。
