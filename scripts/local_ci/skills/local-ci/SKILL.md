...
- Codex 的业务决定只有 `continue` 与 `block`：继续收集证据、调度允许的操作，或依据证据阻塞。检查的 `pass/fail`、审查的 `incomplete`、等待、发布、回执及取消是事实或生命周期状态，继续使用现有结果协议；不得用业务决定替换 `submit_review` 的状态枚举或自行宣布最终通过。

按主流程调用 `finish` 请求 Harness 检查证据并封存。封存后由可信发布器回写结果，只有匹配回执确认后才算完成；恢复流程仍由同一任务的单 Codex 执行。
