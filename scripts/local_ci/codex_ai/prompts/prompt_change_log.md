## 2026-08-25：分离正式验证、诊断命令与公开验证叙述

### 修改原因

可信命令账本原先把 Codex 执行的所有 shell 命令都纳入验证状态，并由 PR comment 按用途统计退出码。搜索、日志检查或环境探查的非零退出码因此可能被误写成整体验证失败，即使正式测试已经通过或后续诊断已经完成。

### 修改内容

- `test_assessment.commands` 增加 `role`，只区分正式验证和诊断；
- `test_execution.status` 只根据正式验证命令派生，完整报告仍保留全部可信命令事实；
- 同一诊断目的只有失败记录时保留未覆盖说明；仅当该限制使整体证据不足时产生 WARNING，存在成功的等价诊断记录时不再保留限制；
- PR comment 的“验证内容与结果”直接使用 Codex 验证摘要，不再把全部命令聚合为固定的成功、失败数量文案；
- 未分类的非零退出命令保守产生报告完整性提醒，但不覆盖正式验证状态，也不导致结构化报告整体失败；公开评论只说明部分辅助检查未形成可确认结果，不展示原始命令。
- PR comment 改为通过公开评论视图读取审查事实，不再直接展示原始命令、任务内部路径、内部关联 ID、结构化字段名或报告完整性诊断；完整报告和 JSON 继续保留这些机器事实。

### 兼容性与风险

- 不修改确定性 Local CI 门禁、Codex 审查范围、预算、镜像或服务器配置；
- 正式验证失败仍影响 Codex verdict，诊断限制通过验证摘要、证据等级和未覆盖范围表达。
