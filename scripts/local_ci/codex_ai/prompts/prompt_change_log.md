## 2026-08-25：公开验证失败说明改为具体语义

### 修改原因

PR comment 原先按内部执行状态拼接固定的失败数量和统一结尾，容易让维护者误以为所有验证都失败，也无法看出失败对应的验证目标、可确认原因及其对整体判断的实际影响。

### 修改内容

- “验证内容与结果”按公开验证用途汇总通过与未完成范围；
- 对失败或受限验证展示可公开的失败观察或原因，并明确它影响的验证目标和整体判断边界；原因尚未确认时如实说明；
- 公开评论不展示原始命令、`RUN-xxx`、退出码、耗时、命令账本或内部状态枚举；这些细节继续保留在结构化 JSON、完整 Markdown 报告和诊断产物中；
- prompt 要求命令 `purpose` 和 `evidence` 提供可公开的人类可读语义，不修改 schema、verdict 或确定性 CI 门禁。

### 兼容性与风险

- 不改变命令执行事实、失败分类、报告语义载荷或完整报告内容；
- 保留公开条目数与评论总长度限制；
- 不新增确定性测试 stage，也不改变 Local CI 的执行范围。
