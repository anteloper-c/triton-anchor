...
runner 使用两个互不重叠的启动边界：`CODEX_AI_CI_PREPARE_TIMEOUT_SECONDS`（默认 1500 秒）限制环境 snapshot、临时容器启动和输入复制的共享总时间，超时报告 `container_prepare_timeout`；容器准备成功后，`CODEX_AI_CI_STARTUP_TIMEOUT_SECONDS`（默认 900 秒）才开始等待首个 `item.started`、`item.completed` 或 `turn.completed` 事件并在超时时报告 `startup_timeout`。出现首个进展后，后续仍由 `CODEX_AI_CI_TIMEOUT_SECONDS` 总时限控制。摘要同时记录 snapshot、容器启动、输入准备和准备总耗时。
