...
| `wheel_install` | 校验 wheel 身份，安装 wheel，隔离 Python 搜索路径后 import | frontend_build |
| `frontend_smoke` | 对已安装 wheel 执行仓库 `tests/test_smoke.py` | wheel_install |
| `backend_rebuild` | 在常驻环境的后端 checkout 重建、安装 wheel，验证后端发现 | wheel_install |
| `backend_smoke` | 后端发现及受信 profile 指定的真实 JIT 测试命令 | backend_rebuild |
...
后六项只适用于 Triton 3.0；其他版本返回 `not_applicable`，明确没有可测后端。
...
* `required_modules`，默认 `build/setuptools/wheel/pybind11`；`required_commands`，
  默认 `git/cmake/ninja`；`required_source_files`，默认构建文件和 frontend smoke。
...
没有匹配显式基线时，选择器在 `state_dir/runs/` 自动查找已经成功发布的 v4
