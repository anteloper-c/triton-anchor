...
def git(root: Path, *args: str) -> str:
    return subprocess.check_output(["git", "-c", "core.fsmonitor=false", "-c", "core.hooksPath=/dev/null", *args], cwd=root).decode("utf-8")
...
def yaml_document(text: str):
    try:
        ...
        raise ValueError("Trusted Python seed requires PyYAML for YAML/workflow contract checks") from exc
...
def workflow_contract(document) -> None:
    ...
    for name, job in jobs.items():
        if not isinstance(name, str) or not re.fullmatch(r'[A-Za-z_][A-Za-z0-9_-]*', name) or not isinstance(job, dict):
            raise ValueError("Invalid workflow job")
        needs = job.get('needs', [])
        needs = [needs] if isinstance(needs, str) else needs
        if not isinstance(needs, list) or any(item not in jobs or item == name for item in needs):
        ...
        if job.get('uses'):
            if not isinstance(job['uses'], str) or 'steps' in job or 'runs-on' in job:
                raise ValueError("Reusable workflow job has incompatible execution fields")
            continue
        if not job.get('runs-on') or not isinstance(job.get('steps'), list) or not job['steps']:
...
def check(root: Path, base: str, tested: str) -> dict:
    ...
    paths = git(root, 'diff', '--name-only', '-z', '--diff-filter=ACMRT', base, tested).split('\0')
    ...
    for relative in filter(None, paths):
        ...
        control = relative.startswith(('.github/', 'scripts/', 'api_contract/')) or path.name in {'pyproject.toml', 'setup.py', 'setup.cfg', '.gitmodules'}
        if suffix not in {'.md', '.rst', '.txt', '.json', '.yaml', '.yml', '.py', '.sh', '.bash', '.toml', '.cfg'} and not control:
            continue
        raw = path.read_bytes()
        if b'\0' in raw:
            continue
        ...
        elif suffix in {'.sh', '.bash'} or re.match(r'^#!.*\b(?:bash|sh)\s*$', text.splitlines()[0] if text else ''):
            subprocess.run(['bash', '-n', str(path)], check=True)
            checks.append('shell_syntax')
        rows.append({'path': relative, 'sha256': hashlib.sha256(raw).hexdigest(), 'checks': checks})
    if not rows and any(not row.startswith('D\t') for row in changed):
        raise ValueError("No supported changed documentation/control file contract was verified")
    return {'status': 'pass', 'base_sha': base, 'tested_sha': tested,
            'diff_check': 'pass', 'changed_files': changed, 'verified_files': rows}
...
def main() -> int:
    ...
    print('Verified diff and', len(result['verified_files']), 'changed documentation/control files')
    return 0
