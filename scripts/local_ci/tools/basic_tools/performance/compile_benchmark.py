...
def build_case(
    kernel: str, torch_mod: Any, flag_gems_mod: Any
...
def time_call(
    fn: Callable[[], Any], torch_mod: Any, flag_gems_mod: Any
) -> tuple[float, Any]:
...
def check_correctness(
    result: Any, reference: Any, torch_mod: Any, rtol: float, atol: float
) -> tuple[bool, float]:
...
def run_child(
    args: argparse.Namespace, kernel: str, phase: str, run_index: int, work_root: Path
) -> dict[str, Any]:
...
def run_parent(args: argparse.Namespace) -> int:
    ...
    if not kernels or args.repeat < 1 or args.warmup < 0:
        raise ValueError(
            "kernels must not be empty; repeat must be positive and warmup non-negative"
        )
