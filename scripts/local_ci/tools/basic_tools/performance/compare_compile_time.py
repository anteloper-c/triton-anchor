...
from common import DEFAULT_KERNELS, load_json  # noqa: E402
...
def compare(
    ...
    if baseline is None:
        warnings.append(
            f"No cached compile-time baseline is available for base SHA {base_sha or '<unknown>'}."
        )
    ...
    for kernel in kernels:
        ...
        if baseline is not None:
            try:
                ...
                if base_ms <= 0:
                    warnings.append(
                        f"Baseline median for {kernel} is not positive: {base_ms:.3f} ms."
                    )
                else:
