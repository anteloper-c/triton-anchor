...
from select_flaggems_tests import Entry, select_entries, write_selected  # noqa: E402
...
def run_operator(
    ...
    if args.clear_cache == "1":
        task_cache = os.environ.get("TRITON_CACHE_DIR")
        if not task_cache:
            raise ValueError("Cache cleanup requires an explicit task-owned TRITON_CACHE_DIR")
        clear_cache_dir(Path(task_cache))
...
def write_reports(
    ...
    document = {
        "schema": "triton-anchor-local-ci/flaggems",
        "mode": args.mode,
