...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=("impact", "sample", "full", "single"), default="impact")
    parser.add_argument("--ops", default="", help="Comma-separated affected operator identifiers")
    parser.add_argument("--categories", default="", help="Comma-separated affected categories")
...
def select_entries(args: argparse.Namespace) -> list[Entry]:
    ...
    if args.mode == "impact":
        ...
        categories = {value for value in getattr(args, "categories", "").split(",") if value}
        ...
        if ops - known_ops or categories - known_categories:
            raise ValueError(f"Unknown FlagGems impact selection: ops={sorted(ops - known_ops)}, categories={sorted(categories - known_categories)}")
        selected = [entry for entry in catalogue if entry.op in ops or entry.marker in ops or entry.category in categories]
