...
PR comment 的“验证情况”不展示 `evidence_level` 或 `test_execution.status` 的内部状态标签，而是直接分为“验证内容与结果”“限制与未覆盖”两组事实。“验证内容与结果”直接展示 Codex 对现有 Local CI 证据、静态审查范围、正式验证、诊断结果和覆盖路径的说明，以及任务级归档的测试文件；不再根据全部 shell 命令的退出码生成固定统计文案。可信 JSONL 命令账本仍完整保留执行事实，`test_assessment.commands.role` 将测试、构建和 lint 等正式验证标记为 `validation`，将搜索、日志检查和环境探查标记为 `diagnostic`；只有正式验证命令参与 `test_execution.status` 派生，未分类命令即使非零退出也不会覆盖正式验证状态。同一诊断目的存在成功记录时允许早先失败由后续等价检查关闭；只有失败记录时保持正式验证状态不变，并在“限制与未覆盖”说明具体诊断目的和证据。公开 renderer 先构造只包含审查事实、影响和验证范围的评论视图，不直接输出原始命令、任务内部路径、内部关联 ID、结构化字段名或报告完整性诊断；未分类非零记录只在公开限制中说明部分辅助检查没有形成可确认结果，完整命令和退出码继续保留在 JSON、完整报告和诊断摘要中。未完成诊断只有在造成实质证据缺口、使 `evidence_level=insufficient` 时才将 Codex verdict 提升为 WARNING；影响轻微且整体证据充分时可以保持 PASS。未执行建议测试和动态验证缺口同样进入“限制与未覆盖”，由缺口产生的具体行为风险才进入“剩余风险”。内部 `evidence_level`、执行状态和完整命令记录继续保留在结构化 JSON、完整报告和诊断摘要中，并独立参与 verdict 计算。
