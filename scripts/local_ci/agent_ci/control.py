...
def validate_control_revision(config: dict, task: dict) -> str:
    ...
    if not Path(__file__).resolve().is_relative_to(root / "scripts/local_ci"):
        raise ContractError("Running worker is outside the configured trusted control checkout")
    try:
    ...
    if revision != task["worker_revision_sha"] or dirty:
        raise ContractError("Installed CI implementation differs from frozen worker_revision_sha; deploy the matching clean revision")
    return revision
