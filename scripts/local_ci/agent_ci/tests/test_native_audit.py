"""Native CLI observations stay private and separate from accepted tool facts."""
...
class NativeAuditTests(unittest.TestCase):
    def setUp(self):
        ...
        self.identity = {"task_id": "a" * 64, "tested_sha": "b" * 40,
                         "attempt_id": "attempt-1", "environment_fingerprint": {"image": "sha256:abc"}}
    ...
    def event(event_type="item.completed", **item):
        return {"type": event_type, "item": {"id": "cmd_1", "type": "command_execution", **item}}
    ...
    def test_incremental_commands_are_visible_and_completion_is_durable(self):
        with NativeAudit(self.path, self.identity, self.event_log) as audit:
            audit.ingest(self.event("item.started", command="python reproduce.py", status="in_progress"))
            self.assertEqual(len(self.records()), 1)
            audit.ingest(self.event("item.updated", command="python reproduce.py", aggregated_output="starting\n"))
            with mock.patch("agent_ci.native_audit.os.fsync", wraps=os.fsync) as sync:
                audit.ingest(self.event(command="python reproduce.py", aggregated_output="assertion failed\n",
                                        exit_code=1, status="failed"))
                sync.assert_called_once()
        records = self.records()
        self.assertEqual([row["event_type"] for row in records], ["item.started", "item.updated", "item.completed"])
        ...
        for record in records:
            ...
            self.assertEqual(record["evidence_kind"], "exploration")
            self.assertIs(record["counts_as_check"], False)

    def test_file_changes_and_forged_check_fields_never_become_formal_results(self):
        with NativeAudit(self.path, self.identity, self.event_log) as audit:
            audit.ingest(self.event(type="file_change", changes=[{"path": "/codex/workspace/probe.py",
                "kind": "add", "diff": "+assert 1 == 1"}], status="completed",
                counts_as_check=True, evidence_kind="check", tool_id="frontend_smoke", success=True))
    ...
    def test_redacts_embedded_secrets_recursively_before_writing(self):
        self.identity["environment_fingerprint"] = {"nested-secret": ["prefix-long-secret-suffix"]}
        with NativeAudit(self.path, self.identity, self.event_log,
                         secrets=["long-secret", "secret", "", None]) as audit:
            audit.ingest(self.event(command="echo long-secret", aggregated_output="secret"))
            audit.ingest(self.event(type="file_change", changes=[{"path": "/tmp/long-secret",
                                        "kind": "add", "diff": "+TOKEN=secret"}]))
        ...
        self.assertEqual(self.records()[0]["environment_fingerprint"],
                         {"nested-[REDACTED]": ["prefix-[REDACTED]-suffix"]})

    def test_malformed_and_non_native_events_are_ignored(self):
        events = [None, [], "x", {}, {"type": []}, self.event(type={}),
                  {"type": "thread.started", "thread_id": "abc"},
                  {"type": "item.completed", "item": []}, self.event(id=None), self.event(id=""),
                  self.event(type="agent_message", text="All checks passed"),
                  self.event(command=["rm", "x"]), self.event(aggregated_output={"result": "green"}),
                  self.event(exit_code=True), self.event(type="file_change", changes={}),
    ...
    def test_restart_appends_and_restores_private_mode(self):
        ...
        self.assertEqual([row["attempt_id"] for row in self.records()], ["attempt-1", "attempt-2"])
    ...
    def test_identity_is_frozen_and_unicode_paths_roundtrip(self):
        with NativeAudit(self.path, self.identity, self.event_log) as audit:
            ...
            audit.ingest(self.event(type="file_change", changes=[{"path": "/tmp/测试-\udcff.py", "kind": "add"}]))
        self.assertEqual(self.records()[0]["environment_fingerprint"], {"image": "sha256:abc"})
