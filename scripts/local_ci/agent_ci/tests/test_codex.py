...
from agent_ci.codex import (
    CodexDriver,
    DISABLED_FEATURES,
    ENABLED_FEATURES,
    finish_timeout_seconds,
)
from agent_ci.credentials import (
    CredentialValidationError,
    parse_toml_fallback,
    validate_credentials,
)
from agent_ci.protocol import ContractError
from agent_ci.program import load_program, Program

CONTROL_ROOT = Path(__file__).resolve().parents[4]
...
class ContainerExecutor:
    ...
    def __init__(self):
        self.generation = {"attempt_id": "fixture-attempt-1", "uids": {"task": 60001}}
        self.uid = 60001
        self.layout = {
            "home": "/task/session/home",
            "workspace": "/task/candidate/checkout",
            "python_bin": "/usr/bin/python3",
            "mcp_script": "/trusted/agent_ci/mcp_server.py",
            "rpc_socket": "/task/rpc/supervisor.sock",
        }
    ...
    def prepare_native_workspace(self):
        return {"root": "/task/candidate", "checkout": "/task/candidate/checkout"}

    def native_environment(self, layout):
        return {
            "PATH": layout["root"] + "/venv/bin:/usr/bin:/bin",
    ...
    def codex_command(self, arguments):
        command = [
            ...
            "--user",
            "60001:60001",
            "owned-task-container",
            "/trusted/private-env-launcher",
            "/trusted/codex",
            *arguments,
        ]
...
@unittest.skipUnless(
    sys.platform.startswith("linux"), "Linux container client boundary"
)
...
""")
        self.source.joinpath("auth.json").write_text(
            json.dumps({"OPENAI_API_KEY": "fixture-private-model-key"})
        )
        ...
        self.supervisor = types.SimpleNamespace(
            observe_native=self.observations.append,
            interrupt_native=self.interruptions.append,
            task={"task_id": "a" * 64},
        ...
        self.service = types.SimpleNamespace(
            path=self.root / "task.sock", token="fixture-private-rpc-capability"
        )
        self.config = {
            "codex_home": str(self.source),
...
"""
        ...
        warnings = []
        self.assertEqual(
            validate_credentials(home, self.root / "personal", warnings=warnings), home
        )
        self.assertEqual(
            parse_toml_fallback(config)["model_providers"]["company"]["wire_api"],
            "responses",
        )
        auth = home / "auth.json"
        auth.unlink()
    ...
    def run_driver(self, **kwargs):
        feature_output = "\n".join(
            name + " stable true" for name in DISABLED_FEATURES | ENABLED_FEATURES
        )
        with (
            mock.patch(
                "agent_ci.codex.subprocess.run",
                return_value=subprocess.CompletedProcess([], 0, feature_output, ""),
            ),
    ...
    def test_new_and_resume_keep_provider_and_mcp_boundary(self):
        ...
        self.assertEqual("/task/candidate/checkout", command[command.index("--cd") + 1])
        ...
        self.assertEqual("/task/rpc/supervisor.sock", env["LOCAL_CI_RPC_SOCKET"])
        ...
        bundle = load_program(CONTROL_ROOT)
        self.assertTrue(
            self.processes[0].input_payload.decode().startswith(bundle.prompt)
        )
        self.assertEqual(bundle.prompt, self.executor.files["AI_CI_PROGRAM.md"])
        ...
        manifest = json.loads((self.run_dir / "program-manifest.json").read_text())
        self.assertEqual(bundle.manifest["sha256"], manifest["sha256"])
        ...
        (self.run_dir / "codex-events.jsonl").write_text(
            json.dumps(
                {
                    "type": "thread.started",
                    "thread_id": "ffffffff-1234-1234-1234-0123456789ab",
                }
            )
            + "\n"
        )
        ...
        self.assertEqual(
            ["exec", "resume", SESSION_ID], command[resume - 1 : resume + 2]
        )
        ...
        self.assertTrue(
            self.processes[1].input_payload.decode().startswith(bundle.prompt)
        )
    ...
    def test_native_command_and_edit_events_are_private_exploration_records(self):
            ...
            events = [
                ...
                },
                {
                    ...
                    "item": {
                        ...
                        "status": "failed",
                        "exit_code": 3,
                        "aggregated_output": "fixture-private-model-key "
                        + self.service.token,
                    },
                },
                {
                    ...
                    "item": {
                        ...
                        "type": "file_change",
                        "status": "completed",
                        "changes": [{"path": "repro.py", "kind": "add"}],
                    },
                },
            ]
            for event in events:
        ...
        with (
            mock.patch(
                "agent_ci.codex.subprocess.run",
                return_value=subprocess.CompletedProcess([], 0, features, ""),
            ),
        ...
        self.assertTrue(all("counts_as_check" not in action for action in actions))
        self.assertTrue(
            all(action["evidence_kind"] == "native_observation" for action in actions)
        )
        self.assertTrue(
            all(
                action["task_id"] == self.supervisor.task["task_id"]
        ...
        forwarded = json.dumps(self.observations)
        self.assertNotIn(self.service.token, forwarded)
        self.assertNotIn("fixture-private-model-key", forwarded)
        self.assertIn("[REDACTED]", forwarded)
        self.assertEqual(["completed"], self.interruptions)

    def test_cancel_finalizes_unfinished_native_observations(self):
        def started_command(command, **kwargs):
            process = self.process(command, **kwargs)
            kwargs["stdout"].write(
                (
                    json.dumps(
                        {
                            "type": "item.started",
                            "item": {
                                "id": "unfinished-native",
                                "type": "command_execution",
                                "command": "python long_test.py",
                                "status": "in_progress",
                            },
                        }
                    )
        ...
        self.assertTrue(
            any(
                e.get("item", {}).get("id") == "unfinished-native"
    ...
    def test_native_export_failure_does_not_erase_event_history(self):
        with mock.patch.object(
            self.executor,
            "export_native_evidence",
            side_effect=ContractError("native evidence export failed"),
        ):
            with self.assertRaisesRegex(ContractError, "native evidence export failed"):
                self.run_driver()
        self.assertEqual(
            SESSION_ID,
            json.loads((self.run_dir / "codex-session.json").read_text())["session_id"],
        )
        self.assertTrue(list(self.run_dir.glob("codex-events-*.jsonl")))
    ...
    def test_finish_budget_uses_cleanup_and_management_contract(self):
        ...
        self.run_driver()
        self.assertEqual(
            "1900", self.executor.environment["LOCAL_CI_FINISH_TIMEOUT_SECONDS"]
        )
    ...
    def test_finish_budget_rejects_invalid_or_insufficient_limits(self):
        for config in (
            {"finish_timeout_seconds": True},
            {"finish_timeout_seconds": 86301},
            {"finish_timeout_seconds": 0},
            {"finish_timeout_seconds": 719},
            {"cleanup_timeout_seconds": True},
            {"management_timeout_seconds": 0},
            {"cleanup_timeout_seconds": 4000},
            {"management_timeout_seconds": "600"},
        ):
            with self.subTest(config=config), self.assertRaises(ContractError):
                finish_timeout_seconds(config)
        self.assertEqual(720, finish_timeout_seconds({"finish_timeout_seconds": 720}))
        self.assertEqual(
            86300, finish_timeout_seconds({"finish_timeout_seconds": 86300})
        )

    def test_missing_program_stops_before_codex_launch(self):
        with mock.patch(
            "agent_ci.codex.load_program", side_effect=ContractError("Missing program")
        ):
            with self.assertRaisesRegex(ContractError, "Missing program"):
                self.run_driver()
        self.assertFalse(self.processes)

    def test_resume_rejects_different_program_or_missing_identity(self):
        self.run_driver()
        path = self.run_dir / "codex-session.json"
        original = json.loads(path.read_text())
        before_manifest = (self.run_dir / "program-manifest.json").read_bytes()
        bundle = load_program(CONTROL_ROOT)
        changed = Program(
            bundle.prompt + "\nNew rule", {**bundle.manifest, "sha256": "0" * 64}
        )
        with mock.patch("agent_ci.codex.load_program", return_value=changed):
            with self.assertRaisesRegex(ContractError, "task/provider/program"):
                self.run_driver()
        self.assertEqual(
            before_manifest, (self.run_dir / "program-manifest.json").read_bytes()
        )
        original.pop("program_digest")
    ...
    def test_session_is_saved_while_codex_is_still_running(self):
        ...
        def wait(seconds):
            observed.append(
                json.loads((self.run_dir / "codex-session.json").read_text())
            )
            return False

        self.supervisor.cancelled = mock.Mock(
            is_set=mock.Mock(return_value=False), wait=mock.Mock(side_effect=wait)
        )
        self.run_driver()
    ...
    def test_replaced_attempt_archives_old_session_and_starts_new(self):
        ...
        self.assertIn(
            "previous task container/session volume was replaced",
            self.processes[1].input_payload.decode(),
        )
        ...
        self.assertEqual(
            "fixture-attempt-2",
            json.loads((self.run_dir / "codex-session.json").read_text())["attempt_id"],
        )
        self.run_driver()
    ...
    def test_sealing_ends_current_codex_process(self):
        ...
        self.supervisor.cancelled = mock.Mock(
            is_set=mock.Mock(return_value=False), wait=mock.Mock(side_effect=seal)
        )
        with (
            mock.patch.object(FakeProcess, "poll", lambda process: process.returncode),
            mock.patch("agent_ci.codex.os.killpg"),
    ...
    def test_model_deadline_still_applies_before_sealing(self):
        self.config["codex_timeout_seconds"] = 10
        self.supervisor.cancelled = mock.Mock(
            is_set=mock.Mock(return_value=False), wait=mock.Mock(return_value=False)
        )
        with (
            mock.patch.object(FakeProcess, "poll", lambda process: process.returncode),
            mock.patch("agent_ci.codex.os.killpg"),
    ...
    def test_nonroot_execution_uid_and_symlinked_credentials(self):
        self.executor.uid = 0
        with self.assertRaisesRegex(ContractError, "non-root"):
            self.run_driver()
        self.executor.uid = 60001
    ...
    def test_removed_editing_flag_is_not_reenabled(self):
        features = (
            "\n".join(
                name + " stable true"
                for name in DISABLED_FEATURES | ENABLED_FEATURES
    ...
    def test_feature_probe_timeout_still_stops_codex_process_group(self):
        with (
            mock.patch(
                "agent_ci.codex.subprocess.run",
                side_effect=subprocess.TimeoutExpired(
                    ["codex", "features", "list"], 30
                ),
            ),
            mock.patch("agent_ci.codex.subprocess.Popen") as popen,
        ):
    ...
    def test_unprivileged_host_never_chowns_or_launches_runuser(self):
        with (
            ...
            mock.patch("os.chown", side_effect=AssertionError("host chown")),
        ):
            self.run_driver()
        self.assertNotIn("runuser", self.processes[0].command)
        self.assertEqual(2, self.executor.stops)

    def test_company_env_credentials_are_private_and_unrelated_secrets_not_inherited(
        self,
    ):
        ...
        with mock.patch.dict(
            os.environ,
            {
                "COMPANY_TOKEN": "fixture-env-secret",
                "GITEE_TOKEN": "never-to-model",
                "LOCAL_CI_RPC_TOKEN": "other-task-token",
            },
        ):
            self.run_driver()
        self.assertEqual(
            "fixture-env-secret", self.executor.environment["COMPANY_TOKEN"]
        )
    ...
    def test_cleanup_failure_propagates_after_retaining_events(self):
        with mock.patch.object(
            self.executor,
            "stop_codex",
            side_effect=[{}, ContractError("Codex process cleanup failed")],
        ):
