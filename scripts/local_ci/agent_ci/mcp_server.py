#!/usr/bin/env python3
"""Task-scoped MCP bridge for all recorded task executions."""
...
DESCRIPTIONS = {
    ...
    "run_custom": "Run task-local Python or Bash; experiment optionally uses a separate source copy. Completed execution can be linked to formal coverage via record_check with actual reports.",
    "submit_review": "Record PR information, architecture or specialized review with verifiable references. High risk findings need code attribution plus reproducible evidence; an approved invariant can replace base comparison where base is inapplicable.",
    "finish": "Validate and seal evidence; missing mandatory checks/reviews prevent pass. Publisher delivers independently.",
    "record_check": "Associate an observed completed native/custom command with actual reports; the same parsers as builtins validate coverage and subject. No execution is repeated just to use an internal tool ID.",
    "cancel_check": "Stop only the selected command process group, preserving the Agent session.",
}
...
SCHEMAS = {
    ...
    "start_check": schema(
        {
            "tool_id": {"type": "string", "pattern": r"^[a-z][a-z0-9_]{0,80}$(?!\s)"},
            "reason": REASON,
    ...
    "run_custom": schema(
        {
            "name": {
                "type": "string",
                "pattern": r"^[A-Za-z0-9][A-Za-z0-9_.-]{0,100}$(?!\s)",
            },
            ...
            "mode": {
                "type": "string",
                "enum": ["diagnostic", "reproduction", "experiment"],
                "description": "Defaults to diagnostic. Only reproduction may establish original-SHA causality.",
            },
            "experiment_id": {
                "type": "string",
                "pattern": r"^[a-f0-9]{32}$(?!\s)",
                "description": "Existing current-task experiment; omit to create one. Allowed only with mode=experiment.",
            },
            "source_only": {
                "type": "boolean",
                "description": "Python-only -I -S using standard library and explicit source reads; no installed packages.",
            },
        },
        ["name", "content", "language", "reason"],
    ),
    "submit_review": schema(
        {
            "kind": {
                "type": "string",
                "enum": ["pr_info", "architecture", "specialized"],
            },
        ...
        ["kind", "status", "summary", "evidence"],
    ),
    "finish": schema({"summary": S}, []),
}
SCHEMAS.update(
    {
        "cancel_check": schema({"execution_id": EXECUTION_ID}, ["execution_id"]),
        "record_check": schema(
            {
                "execution_id": EXECUTION_ID,
                "tool_id": NONEMPTY,
            ...
            ["execution_id", "tool_id", "artifact_path", "reason"],
        ),
    }
)
...
def validate_value(value, shape: dict, path: str) -> None:
    ...
    if shape.keys() - supported or isinstance(shape.get("additionalProperties"), dict):
        raise ArgumentValidationError(
            f"{path} uses an unsupported server schema constraint"
        )
    ...
    if expected is not None and (
        expected not in kinds or type(value) is not kinds[expected]
    ):
    ...
    if isinstance(value, dict):
        ...
        if (
            len(value) < shape.get("minItems", 0)
            or "maxItems" in shape
            and len(value) > shape["maxItems"]
        ):
        ...
        if (
            len(value) < shape.get("minLength", 0)
            or "maxLength" in shape
            and len(value) > shape["maxLength"]
        ):
        ...
        if (
            "minimum" in shape
            and value < shape["minimum"]
            or "maximum" in shape
            and value > shape["maximum"]
        ):
...
def validate_arguments(method: str, arguments: dict) -> None:
    ...
    if method == "run_custom":
        if (
            "experiment_id" in arguments
            and arguments.get("mode", "diagnostic") != "experiment"
        ):
            raise ArgumentValidationError(
                "experiment_id is only valid in experiment mode"
            )
...
def call(method: str, arguments: dict) -> dict:
    ...
    with socket.socket(socket.AF_UNIX) as client:
        ...
        timeout = (
            int(os.environ.get("LOCAL_CI_FINISH_TIMEOUT_SECONDS", "3240"))
            if method == "finish"
            else 60
        )
        ...
        request = {
            "token": os.environ["LOCAL_CI_RPC_TOKEN"],
            "method": method,
            "arguments": arguments,
        }
    ...
    try:
        if method == "initialize":
            ...
            result = {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "local-ci", "version": "4.0"},
            }
        elif method == "ping":
            result = {}
                ...
                "content": [
                    {
                        "type": "text",
                        "text": json.dumps(
                            reply.get("result", reply), ensure_ascii=False
                        ),
                    }
                ],
