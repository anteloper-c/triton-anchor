...
DISABLED_FEATURES = {
    ...
    "memories",
    "shell_snapshot",
    "program_mcp_dependency_install",
    "code_mode_host",
    "js_repl",
    "python_repl",
}
...
def finish_timeout_seconds(config: dict) -> int:
    """Bound the entire seal RPC: task execution groups and pending evidence.
    ...
    one process-group cleanup, one management export and local sealing time.
    """
    ...
    if any(type(value) is not int or value < 1 for value in (cleanup, management)):
        raise ContractError(
            "Process cleanup and management timeouts must be positive integers"
        )
    if type(seconds) is not int or not 1 <= seconds <= 86300:
        raise ContractError(
            "finish_timeout_seconds must be an integer between 1 and 86300"
        )
    if seconds < cleanup + management + 60:
        raise ContractError(
            "finish_timeout_seconds must cover one process cleanup, one management export and 60 seconds of sealing"
        )
    return seconds
...
def toml_value(value) -> str:
    if isinstance(value, dict):
        return (
            "{ "
            + ", ".join(
                json.dumps(str(k)) + " = " + toml_value(v) for k, v in value.items()
            )
            + " }"
        )
...
class CodexDriver:
    ...
    def safe_path(path: Path) -> None:
        if not path.is_absolute() or path != path.resolve() or path.is_symlink():
            raise ContractError(
                "Codex credential/session paths must be absolute without symlink components"
            )
    ...
    def run(self, supervisor, service, *, recovery: str = "") -> dict:
        ...
        if supervisor.cancelled.is_set():
            return {
                "exit_code": None,
                "reason": "cancelled",
                "finished": supervisor.closed,
                "session_id": None,
            }
        program_entry = load_program(
            Path(self.config.get("control_root") or Path(__file__).resolve().parents[3])
        )
        source = Path(
        ...
        if not all(
            ...
            raise ContractError(
                "Dedicated company Codex config.toml/auth.json are required"
            )
        ...
        if (
            not settings.get("model")
            or not provider.get("base_url")
            or provider.get("wire_api", "responses") != "responses"
        ):
            raise ContractError(
                "Use the deployed company Responses provider and actual model"
            )
        auth = json.loads((source / "auth.json").read_text())
        if not isinstance(auth, dict) or not (
            auth.get("OPENAI_API_KEY") or auth.get("tokens")
        ):
            raise ContractError(
                "Dedicated company auth.json contains no authentication material"
            )
        executor = supervisor.executor
        if executor.uid <= 0:
            raise ContractError("Task execution user must be non-root")
        ...
        if not isinstance(attempt_id, str) or not re.fullmatch(
            r"[A-Za-z0-9][A-Za-z0-9_.-]{0,119}", attempt_id
        ):
        ...
        if saved.exists():
            ...
            if (
                ...
                or value.get("program_digest") != program_entry.manifest["sha256"]
                or not UUID.fullmatch(value.get("session_id", ""))
            ):
                raise ContractError(
                    "Saved Codex session identity differs from this task/provider/program; resume with its trusted control revision"
                )
            if value.get("attempt_id") == attempt_id:
                ...
                atomic_json(
                    ...
                    {
                        **value,
                        "archived_reason": "task_attempt_replaced",
                        "replacement_attempt_id": attempt_id,
                    },
                )
                recovery = (
                    "The previous task container/session volume was replaced. Start a new Codex session; "
                    "read context and revalidate invalidated checks in the current attempt. "
                    + recovery
                )
        layout = executor.prepare_codex_session(
            files={}, environment={}, rpc_socket=Path(service.path)
        )
        native = executor.prepare_native_workspace()
        home, workspace = layout["home"], native["checkout"]
        program = program_entry.prompt
        atomic_json(
            supervisor.run_dir / "program-manifest.json",
            {"task_id": task_id, **program_entry.manifest},
        )
        # Preserve the deployed model/provider, not unrelated hooks or MCP servers.
        effective = {
            key: value for key, value in settings.items() if key in MODEL_SETTINGS
        }
        finish_timeout = finish_timeout_seconds(self.config)
        effective.update(
            {
                "model_providers": {provider_name: provider},
                ...
                "mcp_servers": {
                    "local_ci": {
                        ...
                        "env_vars": [
                            "LOCAL_CI_RPC_SOCKET",
                            "LOCAL_CI_RPC_TOKEN",
                            "LOCAL_CI_FINISH_TIMEOUT_SECONDS",
                        ],
                        "required": True,
                        "enabled": True,
                        "enabled_tools": sorted(SCHEMAS),
                        "startup_timeout_sec": 30,
                        "tool_timeout_sec": finish_timeout + 30,
                    }
                },
            }
        )
        child_env = {
            **executor.native_environment(native),
            "CODEX_HOME": str(home),
            "LANG": "C.UTF-8",
            "LOCAL_CI_CODEX_WORKSPACE": str(workspace),
            "LOCAL_CI_RPC_SOCKET": layout["rpc_socket"],
        ...
        for key in provider_env:
            if not isinstance(key, str) or key in child_env or key not in os.environ:
                raise ContractError(
                    "Required company provider environment is unavailable"
                )
        ...
        for key in self.config.get("codex_proxy_env", []):
            ...
            if key in os.environ:
                child_env[key] = os.environ[key]
        files = {
            "config.toml": config_text(effective),
            "auth.json": json.dumps(auth),
            "AI_CI_PROGRAM.md": program,
        }
        executor.prepare_codex_session(
            files=files, environment=child_env, rpc_socket=Path(service.path)
        )
        client_env = self.client_environment()
        # CLI capability inspection is local and never calls a model.
        try:
            features = subprocess.run(
                executor.codex_command(["features", "list"]),
                cwd=supervisor.run_dir,
                env=client_env,
                capture_output=True,
                text=True,
                timeout=30,
        ...
        if features.returncode or not REQUIRED_FEATURES <= supported:
            raise ContractError(
                "Deployed Codex CLI cannot enforce the required tool boundary"
            )
        effective["features"] = {
            name: False for name in sorted(DISABLED_FEATURES & supported)
        }
        effective["features"].update(
            {name: True for name in sorted(ENABLED_FEATURES & supported)}
        )
        executor.prepare_codex_session(
            files={"config.toml": config_text(effective)},
            environment=child_env,
            rpc_socket=Path(service.path),
        )
        ...
        prompt += "\n当前任务工作区（原生命令可关联正式证据）：" + json.dumps(
        ...
        atomic_json(
            native_root / "context.json",
            {
                "task_id": task_id,
                "attempt_id": attempt_id,
                "tested_sha": supervisor.task.get("tested_sha"),
                "event_log": str(output),
            },
        )
        ...
        private_values = [
            *credential_strings(auth),
            *credential_strings(provider.get("http_headers", {})),
            *(child_env[k] for k in provider_env),
            *(
                child_env[k]
                for k in self.config.get("codex_proxy_env", [])
                if k in child_env
            ),
        ]
        ...
        audit = NativeAudit(
            audit_path,
            {
                ...
                "tested_sha": supervisor.task.get("tested_sha"),
                "attempt_id": attempt_id,
                "environment_fingerprint": executor.generation.get(
                    "environment_fingerprint"
                ),
            },
            output,
            secrets=[service.token, *private_values],
        )
        supervisor.redact = audit.redact
        ...
        def consume_events():
            ...
            for line in lines:
                try:
                    ...
                    continue
                audit.ingest(event)
                supervisor.observe_native(audit.redact(event))
                if (
                    isinstance(event, dict)
                    and event.get("type") == "thread.started"
                    and UUID.fullmatch(str(event.get("thread_id", "")))
                ):
                    ...
                    atomic_json(
                        saved,
                        {
                            ...
                            "program_digest": program_entry.manifest["sha256"],
                            "attempt_id": attempt_id,
                        },
                    )
        ...
        with audit, output.open("wb") as stream:
            ...
            process = subprocess.Popen(
                executor.codex_command(command),
                cwd=supervisor.run_dir,
                env=client_env,
            ...
            try:
                ...
                while process.poll() is None:
                    ...
                    if getattr(supervisor, "sealing_started", False):
                        ...
                        deadline = started + self.config.get(
                ...
                try:
                    ...
                    executor.stop_codex()
                    exported = executor.export_native_evidence(
                        native_root / "workspace"
                    )
                    if not isinstance(exported, dict) or not (
                        exported.get("exported") or exported.get("evidence_loss")
                    ):
                        raise ContractError(
                            "Native evidence export did not report completion"
                        )
                    ...
                    consume_events()
                    supervisor.interrupt_native(reason)
        return {
            ...
            "native_evidence": str(native_root / "workspace"),
            "program_digest": program_entry.manifest["sha256"],
        }
