...
from .artifacts import (
    EnvironmentError, SHA_RE, atomic_json, extract_verified_archive,
    file_digest, tree_digest,
)
    ...
    with (directory / ".lock").open("a") as lock:
        ...
        if root.exists() or manifest.exists():
            try:
                ...
                raise EnvironmentError("Control snapshot cache is incomplete or changed") from exc
        with tempfile.TemporaryDirectory(prefix=".snapshot-", dir=directory) as temporary:
            ...
            paths = run(["git", "-C", str(control_root), "ls-tree", "--name-only",
                         revision, "--", "scripts", "api_contract", "envsetup.sh"]).decode().splitlines()
            if "scripts" not in paths:
                raise EnvironmentError("Committed control scripts are missing")
            run(["git", "-C", str(control_root), "archive", "--format=tar",
                 "--output=" + str(archive), revision, "--", *paths])
...
def mount_arguments(snapshot):
    return ["--mount", "type=bind,source=" + snapshot["source"]
            + ",target=" + CONTROL_TARGET + ",readonly,bind-recursive=disabled"]


def verify_mount(info, snapshot):
    mounts = [m for m in info.get("Mounts", []) if m.get("Destination") == CONTROL_TARGET]
    if (len(mounts) != 1 or mounts[0].get("Type") != "bind"
            or mounts[0].get("Source") != snapshot["source"] or mounts[0].get("RW") is not False):
        raise EnvironmentError("Control snapshot mount identity or readonly mode changed")
