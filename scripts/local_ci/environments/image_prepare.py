...
def prepare_image():
    ...
    if llvm["mode"] == "source":
        ...
        arguments = llvm.get("cmake_args", [
            "-G", "Ninja", "-DCMAKE_BUILD_TYPE=Release",
            "-DLLVM_ENABLE_PROJECTS=mlir;clang;lld",
            "-DLLVM_TARGETS_TO_BUILD=host;NVPTX;AMDGPU",
        ])
        subprocess.run(["cmake", "-S", source, "-B", build, *arguments,
                        "-DCMAKE_INSTALL_PREFIX=" + environment["LLVM_BUILD_DIR"]],
                       check=True, env=environment)
        subprocess.run(["cmake", "--build", build, "--target", "install",
                        "--parallel", str(recipe.get("build_jobs", 8))],
                       check=True, env=environment)
        shutil.rmtree(build)
        shutil.rmtree("/opt/local-ci/runtime/deps/llvm-source")
    for command in recipe.get("prepare_commands", []):
        subprocess.run(command, check=True, env=environment, cwd="/opt/local-ci/runtime")
    root = Path("/opt/local-ci/runtime")
    # COPY can inherit a private host umask; all four container roles need read access.
