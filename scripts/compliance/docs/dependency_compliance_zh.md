...
提交 `bc67046` 进一步为 Wheel ELF 已实际识别出的 glibc 运行库查询同次 Hosted Runner 的 `libc6` 二进制包、Ubuntu 源包版本和 OSV 身份。普通运行 `33617609706` 和手工 audit `33619224129` 已验证 Ubuntu 24.04 的 `libc6:amd64` 来自 `glibc` 源包，版本 `2.39-0ubuntu8.8`，OSV 覆盖为 `scanned`；完整 audit 中 glibc 的 inventory 缺口因此只剩“approved concluded license”，仍须人工批准。该次 OSV 原始证据还观察到 CPython 3.12.14 的中等和未定严重度记录；它们没有触发当前高风险漏洞门禁，但仍是后续审计需要跟踪的事实。当前分支继续为 `gcc-runtime` 同时查询提供 `libgcc_s.so.1` 与 `libstdc++.so.6` 的 `libgcc-s1`、`libstdc++6`；只有两个二进制包解析到相同 Ubuntu 源包、版本和 OSV 生态时才接受该组件身份，在新的 Hosted Runner audit 完成前只能表述为已实现并通过本地测试。可选 Python 运行依赖和 test-only 依赖也只能由以后明确选定的支持环境提供版本，不能登记任意最新版本。
