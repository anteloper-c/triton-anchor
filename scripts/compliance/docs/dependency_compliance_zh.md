...
当前分支进一步为 Wheel ELF 已实际识别出的 glibc 运行库查询同次 Hosted Runner 的 `libc6` 二进制包、Ubuntu 源包版本和 OSV 身份；在新的 Hosted Runner audit 完成前，这一项只能表述为已实现并通过本地构建证据测试，尚未远端验证。`gcc-runtime` 同时覆盖 `libgcc_s.so.1` 与 `libstdc++.so.6`，不能选其中一个二进制包冒充完整来源；可选 Python 运行依赖和 test-only 依赖也只能由以后明确选定的支持环境提供版本，不能登记任意最新版本。
